...
# Start 220 bytes before G388 so original 256-byte donor alignment is retained.
dd if="$full_stream" of="$work_root/g388_g389.postr1" \
  bs=1M skip=406789888 count=2090415 \
...
printf '%s\n' \
  ...
  '1049042,1041373,donor_profile,url,0,0.25,' > "$work_root/experts.csv"
