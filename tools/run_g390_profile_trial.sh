...
readonly work_root=/root/fx4_g390_profile_20260812

mkdir -p "$work_root"
exec 9>"$work_root/run.lock"
flock -n 9 || { echo "G390 trial is already running" >&2; exit 3; }
...
# 256-byte-aligned continuous G388 -> G390 post-R1 slice.
dd if="$full_stream" of="$work_root/g388_g390.postr1" \
  bs=1M skip=406789888 count=3133705 \
...
printf '%s\n' \
  ...
  '0,1003776,4096,4' \
  '0,1009920,4096,5' \
  '0,1020160,2048,6' > "$work_root/donors.csv"

printf '%s\n' \
  'offset,length,experts,stream_class,profile,residual_gain,mini_models' \
  '2090415,1043290,donor_profile,mixed,0,0.25,' > "$work_root/experts.csv"

python3 "$source_root/tools/build_postr1_portfolio.py" \
  "$work_root/g388_g390.postr1" \
  --prefix "$work_root/g390_profile" \
...
/usr/bin/time -v -o baseline.time \
  ...
  taskset -c 7 ./cmix -n english.dic g388_g390.postr1 baseline.fx4
...
/usr/bin/time -v -o profile.time \
  env FX4_RAW_ENTROPY_INPUT=1 FX4_FORCE_FULL_VOCAB=1 \
      FX4_DONOR_PLAN="$work_root/g390_profile.f4cp" \
  taskset -c 7 ./cmix -n english.dic g388_g390.postr1 profile.fx4
...
if (( candidate < baseline )); then
  ...
  cmp g388_g390.postr1 restored.postr1
