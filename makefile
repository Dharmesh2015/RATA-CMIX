...
DONOR_DISCOVERY_SOURCE := src/donor_fork_discovery.cpp
DONOR_DISCOVERY_HEADER := src/donor_fork_discovery.h
DONOR_DISCOVERY_OBJECT := donor_fork_discovery.o
endif
DONOR_SOURCE :=
DONOR_HEADER :=
DONOR_OBJECT :=
...
cold: $(DONOR_SOURCE) $(DONOR_HEADER) $(DONOR_DISCOVERY_SOURCE) $(DONOR_DISCOVERY_HEADER) src/r1_reorder_transform.cpp src/r1_reorder_transform.h src/runner.cpp
	$(CC) $(CPPFLAGS_PART-THAT-IS-COLD) $(DONOR_SOURCE) $(DONOR_DISCOVERY_SOURCE) src/r1_reorder_transform.cpp src/runner.cpp -c
...
cmix: fast slow cold
	$(CC) $(LFLAGS) bit-context.o bracket-context.o bracket.o byte-mixer.o byte-model.o combined-context.o context-hash.o context-manager.o decoder.o dictionary.o direct-hash.o direct.o $(DONOR_OBJECT) $(DONOR_DISCOVERY_OBJECT) encoder.o indirect-hash.o interval-hash.o interval.o match.o mixer-input.o mixer.o nonstationary.o fxcmv1.o ppmd.o predictor.o preprocessor.o r1_reorder_transform.o run-map.o runner.o sigmoid.o sparse.o sse.o $(STRIP_FLAG) -o $(OUT)
	rm -f *.o
